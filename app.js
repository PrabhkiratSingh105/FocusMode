...
const defaultState = {
  settings: { pomodoroMinutes: 25, countdownMinutes: 15, theme: 'aurora' },
  timerMode: 'pomodoro',
  history: [],
  breaks: [],
  todos: [],
...
function bindSettings() {
  ui.openSettings.addEventListener('click', () => {
    ...
    ui.themeSelect.value = state.settings.theme || 'aurora';
  ...
  ui.saveSettingsBtn.addEventListener('click', (e) => {
    ...
    state.settings.theme = ui.themeSelect.value || 'aurora';
    applyTheme();
    timerSeconds = state.settings[`${state.timerMode}Minutes`] * 60;
    updateTimerDisplay();
    saveState();
...
function applyTheme() {
  document.body.classList.remove('theme-midnight', 'theme-nebula');
  if (state.settings.theme === 'midnight') document.body.classList.add('theme-midnight');
  if (state.settings.theme === 'nebula') document.body.classList.add('theme-nebula');
}
